'use client';

import { ZodProvider } from '@autoform/zod';
import { useTranslations } from 'next-intl';
import { toast } from 'sonner';
import { AutoForm } from '@/components/autoform';
import { Button } from '@/components/ui/button';
import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog';
import { FormMode } from '@/types/form';
import type {
  CreatePromocodeFormInput,
  EditPromocodeFormInput,
  Promocode,
} from '@/features/promocodes';
import {
  createPromocodeFormSchema,
  editPromocodeFormSchema,
  useCreatePromocode,
  useUpdatePromocode,
} from '@/features/promocodes';
import { Pencil, PlusCircle } from 'lucide-react';
import { useState } from 'react';

interface PromocodeFormDialogProps {
  mode: FormMode;
  promocode?: Promocode | null;
}

const modeConfig = {
  create: {
    buttonTrigger: 'createNew',
    title: 'createTitle',
    description: 'createDescription',
    idleSubmitText: 'createNew',
    busySubmitText: 'creating',
    buildSchema: createPromocodeFormSchema,
  },
  edit: {
    buttonTrigger: 'actionEdit',
    title: 'editTitle',
    description: 'editDescription',
    idleSubmitText: 'save',
    busySubmitText: 'saving',
    buildSchema: editPromocodeFormSchema,
  },
};

function toDateInputValue(value?: string) {
  if (!value) return '';

  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return '';
  return date.toISOString().slice(0, 10);
}

export function PromocodeFormDialog({
  mode,
  promocode,
}: PromocodeFormDialogProps) {
  const t = useTranslations('promocodes');
  const currentModeConfig = modeConfig[mode];
  const schema = new ZodProvider(currentModeConfig.buildSchema(t));
  const createMutation = useCreatePromocode();
  const updateMutation = useUpdatePromocode();
  const isBusy = createMutation.isPending || updateMutation.isPending;
  const [open, setOpen] = useState(false);

  const handleSubmit = async (
    values: CreatePromocodeFormInput | EditPromocodeFormInput,
  ) => {
    try {
      if (mode === 'create') {
        await createMutation.mutateAsync(values);
        toast.success(t('createSuccess'));
        setOpen(false);
        return;
      }

      if (mode === 'edit' && promocode) {
        const { code: _code, ...payload } = values;
        await updateMutation.mutateAsync({
          id: promocode.id,
          payload,
        });
        toast.success(t('editSuccess'));
        setOpen(false);
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('createError'));
    }
  };

  return (
    <Dialog open={open} onOpenChange={(nextOpen) => setOpen(nextOpen)}>
      <DialogTrigger asChild>
        <div>
          {mode === 'create' && (
            <Button size='sm'>
              <PlusCircle className='mr-2 size-4' />
              {t(currentModeConfig.buttonTrigger)}
            </Button>
          )}

          {mode === 'edit' && (
            <Button variant='ghost' size='icon'>
              <Pencil className='h-4 w-4' />
            </Button>
          )}
        </div>
      </DialogTrigger>

      <DialogContent className='sm:max-w-lg'>
        <DialogHeader>
          <DialogTitle>{t(currentModeConfig.title)}</DialogTitle>
          <DialogDescription>
            {t(currentModeConfig.description)}
          </DialogDescription>
        </DialogHeader>

        <AutoForm
          key={promocode?.id ?? mode}
          schema={schema}
          formProps={{
            className: 'grid grid-cols-6 gap-4',
          }}
          defaultValues={{
            code: promocode?.code ?? '',
            name: promocode?.name ?? '',
            description: promocode?.description ?? '',
            discountType: promocode?.discountType ?? 'fixed',
            discountValue: promocode?.discountValue ?? 0,
            maxAmount: promocode?.maxAmount ?? 0,
            minSpend: promocode?.minSpend ?? 0,
            startDate: toDateInputValue(promocode?.startDate),
            endDate: toDateInputValue(promocode?.endDate),
            usageLimit: promocode?.usageLimit ?? 0,
            isActive: promocode?.isActive ?? true,
          }}
          onSubmit={handleSubmit}
        >
          <div className='col-span-full flex justify-end gap-2 pt-2'>
            <DialogClose asChild>
              <Button type='button' variant='outline' disabled={isBusy}>
                {t('cancel')}
              </Button>
            </DialogClose>
            <Button type='submit' disabled={isBusy}>
              {isBusy
                ? t(currentModeConfig.busySubmitText)
                : t(currentModeConfig.idleSubmitText)}
            </Button>
          </div>
        </AutoForm>
      </DialogContent>
    </Dialog>
  );
}
