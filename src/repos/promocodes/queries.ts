import { ObjectId } from 'mongodb';
import type { PagedList } from '@/types/api';
import { getPromocodesCollection } from './collection';
import { mapPromocodeDocument } from './mapper';
import type { PromoCode, PromoCodePagedParams } from './types';

export async function findPromocodesPaged({
  page,
  pageSize,
  keyword,
  isActive,
  discountType,
}: PromoCodePagedParams): Promise<PagedList<PromoCode>> {
  const collection = await getPromocodesCollection();
  const filter: Record<string, unknown> = {};

  if (typeof isActive === 'boolean') {
    filter.isActive = isActive;
  }

  if (discountType === 'percent' || discountType === 'fixed') {
    filter.discountType = discountType;
  }

  if (keyword) {
    filter.$or = [
      { code: { $regex: keyword, $options: 'i' } },
      { name: { $regex: keyword, $options: 'i' } },
      { description: { $regex: keyword, $options: 'i' } },
    ];
  }

  const totalCount = await collection.countDocuments(filter);
  const totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
  const normalizedPage = Math.min(Math.max(page, 1), totalPages);
  const skip = (normalizedPage - 1) * pageSize;

  const docs = await collection
    .find(filter)
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(pageSize)
    .toArray();

  return {
    items: docs.map(mapPromocodeDocument),
    totalCount,
    page: normalizedPage,
    pageSize,
    totalPages,
    hasNextPage: normalizedPage < totalPages,
    hasPreviousPage: normalizedPage > 1,
  };
}

export async function findPromocodeById(id: string) {
  if (!ObjectId.isValid(id)) return null;

  const collection = await getPromocodesCollection();
  const doc = await collection.findOne({ _id: new ObjectId(id) });
  return doc ? mapPromocodeDocument(doc) : null;
}
