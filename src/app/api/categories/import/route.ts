...
import { createCategorySchema, insertCategory } from '@/repos/categories';
...
interface ImportCategoriesResult {
  totalRows: number;
...
interface ImportCategoriesOutput extends ImportCategoriesResult {
  headers: string[];
...
function toSlugValue(value: unknown): string {
...
function getFirstValidationMessage(
  error: ReturnType<typeof createCategorySchema.safeParse>,
) {
...
function buildCategoryPayloadFromRow(row: ExcelRow) {
  ...
  const slugFromSheet = toStringValue(getCell(row, ['slug', 'duongDan', 'ma']));
  const slug = toSlugValue(slugFromSheet || name);
...
function isPayloadEmpty(
  payload: ReturnType<typeof buildCategoryPayloadFromRow>,
) {
  return (
...
function localizeRowMessage(
  ...
  if (locale === 'vi') {
    ...
    if (code === 'duplicate_category')
      return 'Tên hoặc slug danh mục đã tồn tại';
  ...
  if (code === 'duplicate_category')
    return 'Category slug or name already exists';
...
async function createResultFile(
  ...
  return {
    resultFileName: `categories-import-result-${Date.now()}.xlsx`,
    resultFileBase64,
  };
}
...
async function processImportRows(
...
): Promise<ImportCategoriesOutput> {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const payload = buildCategoryPayloadFromRow(row);
    const excelRowNumber = index + 2;
